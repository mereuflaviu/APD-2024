...
public class Reader extends Thread {
    ...
    public void run() {
        ...
        for (int i = 0; i < number_of_reads; i++) {
            try {
                // Start reader access synchronization
                shared_vars.readersMutex.acquire();
                if (shared_vars.readers == 0) {
                    shared_vars.readWrite.acquire(); // First reader locks the resource
                }
                shared_vars.readers++;
                shared_vars.readersMutex.release();

                // Perform reading
                read();

                // End reader access synchronization
                shared_vars.readersMutex.acquire();
                shared_vars.readers--;
                if (shared_vars.readers == 0) {
                    shared_vars.readWrite.release(); // Last reader releases the resource
                }
                shared_vars.readersMutex.release();
