...
int* get_dst(int rank, int numProcs, int leader) {
	...
	*/
	...
	for (int i = 0; i < num_neigh; i++)
	{
		if (neigh[i] != v[rank])
		{
			MPI_Recv(vRecv, numProcs, MPI_INT, neigh[i], TAG_ECOU, MPI_COMM_WORLD, &status);
	...
	*/
	if (rank != leader)
	{
        ...
        MPI_Recv(vRecv, numProcs, MPI_INT, v[rank], TAG_ECOU, MPI_COMM_WORLD, &status);
	}
...
int ** get_topology(int rank, int nProcesses, int * parents, int leader) {
	...
	for (int i = 0; i < num_neigh; i++) {
        if (parents[neigh[i]] == rank) {
            MPI_Recv(&(vTopology[0][0]), nProcesses * nProcesses, MPI_INT, neigh[i], 0, MPI_COMM_WORLD, MPI_STATUS_IGNORE);
	...
	for (int i = 0; i < num_neigh; i++) {
        if (parents[neigh[i]] == rank) {
            MPI_Send(&(topology[0][0]), nProcesses * nProcesses, MPI_INT, neigh[i], 0, MPI_COMM_WORLD);
        }
    }
